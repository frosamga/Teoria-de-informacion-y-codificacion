...
public class BMPHandler {
	...
	public static int[][][] loadBMP(String openName) {
		Image img = (Image) null; // create a null Image

		img = loadbitmap("./", openName);
		...
		if (img == null) {
			...
			System.out
					.println("Make sure that you supply the name of an image file, \nand that you include the bmp extension.");
			return null;
		} // if (img==null)
	...
	public void saveImage(String saveName, int[][][] imagePixels) {
		...
		for (int j = 0; j < flat.length; j++) {
			outPixels[j] = ((flat[j][0] & 0xff) << 16)
					| ((flat[j][1] & 0xff) << 8) | (flat[j][2] & 0xff)
					| ((flat[j][3] & 0xff) << 24);
		} // for j

		// Write the data out to file with the name given by string saveName.
		saveBitmap(saveName, outPixels, width, height);
	...
	public static Image loadbitmap(String sdir, String sfile) {
		...
		try {
			...
			if (nbitcount == 24) {
						...
						if (i1 * 8 < nwidth) {
							for (int b1 = 128; b1 > 0; b1 = b1 / 2) {
								ndata1[nwidth * (nheight - j1 - 1) + i1 * 8
										+ ib1] = ((b1 & bdata[iindex]) > 0) ? 255 + (255 + 255 * 256) * 256
										: 0;
								ib1++;
								if (i1 * 8 + ib1 >= nwidth) {
									b1 = 0;
				...
				System.out
						.println("Not a 24-bit or 8-bit or 1-bit Windows Bitmap, aborting...");
				image = (Image) null;
			}
	 ...
	 * @return
	 */
	private static int[] getPixels(Image parImage, int parWidth, int parHeight) {
	 ...
	 * @param parHeight
	 */
	private void saveBitmap(String parFilename, int[] imagePix, int parWidth,
			int parHeight) {
	...
	private void writeBitmap() {
		...
		// The following bug correction will cause the bitmap to be unreadable
		// by
	...
	/****************************
	 * Metodo privado que convierte un int a word y devuelve el valor en un
	 * array de 2 bytes.
	 * 
	 * Utilizado en writeBitmapInfoHeader y writeBitmapFileHeader
	 * 
	 * @param parValue
	 * @return
	 */
	...
	/****************************
	 * Metodo privado que convierte un int a double word y lo devulve en un
	 * array de 4 bytes.
	 * 
	 * Utilizado en writeBitmapInfoHeader
	 * 
	 * @param parValue
	 * @return
	 */
