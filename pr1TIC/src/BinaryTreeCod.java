...
public class BinaryTreeCod {
	...
	public boolean buildCodeParameters(int[] n) throws TreeException {
		...
		for (int i = 0; i < size; i++) {
			if (n[i] > 0)
				result = levelFreePrefix(i + 1, n[i]); // Eliminate unnecessary
														// invocation, 0
														// elements level
			if (!result)
				return false;
	...
	private int levelAuxPrefixFree(Node focusNode, int level, int usedLevel) {
		...
		if (level == 1) {
			...
			if (usedLevel > 0 & !focusNode.leftChild.isUsed()) {
				usedLevel = levelAuxPrefixFree(focusNode.leftChild, level - 1,
						usedLevel);
			}
			if (usedLevel > 0 & !focusNode.rightChild.isUsed()) {
				usedLevel = levelAuxPrefixFree(focusNode.rightChild, level - 1,
						usedLevel);
			}
		}
	...
	/**********
	 * Imprime los parametros generados. ToDo: parameters puede ser eliminados
	 * para no evitar confusiones. Deben ser los mismos que los utilizados para
	 * construir el arbol. Se pueden almacenar como atributo de la clase.
	 * 
	 * @param parameters
	 */
	 ...
	 * 
	 * @param parameters
	 *            . Crear codigo segun unos parametros.
	 * @param printTree
	 *            . Decido si quiero imprimir o no el arbol.
	 */
	static public void Test(int[] parameters, boolean printTree) {
		...
		try {
			...
			if (result) {
				System.out
						.println("BinaryTreeMethod:Found prefix free code in "
								+ (fin - ini) + " nanoseconds");
				...
				System.out.println("Verifying prefix free: " + isPrefixFree
						+ " in " + (fin - ini) + " nanoseconds");

			} else
				System.out
						.println("BinaryTreeMethod: I can not generate prefix free code usign this parameters!!. Loosing "
								+ (fin - ini) + " nanosecodns");
		...
		System.out.println("kraft- McMillan Regular      Value: " + KMvalue
				+ " in " + (fin - ini) + " nanoseconds");
		...
		System.out.println("kraft- McMillan Optimization Value: " + KMvalue
				+ " in " + (fin - ini) + " nanoseconds");
